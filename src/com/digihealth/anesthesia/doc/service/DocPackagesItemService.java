...
public class DocPackagesItemService extends BaseService {
	...
	public List<DocPackagesItem> queryPackagesItemList(SystemSearchFormBean systemSearchFormBean,int type, BigDecimal chargeShouldSum){
		/*String filter = "";
        ...
        if(resultList!=null && resultList.size()>0){
            for(int i = 0 ; i < resultList.size();i++){
                ...
                chargeShouldSum.add(new BigDecimal(packageItem.getShouldSum()));
            }
        }
        return packagesItemList;
	}
	...
	public void  updateChargeState(List<OperCost> costList){
		if(null!=costList && costList.size()>0){
			for (OperCost operCost : costList) {
				docPackagesItemDao.updateChargeState(operCost.getPkItId());
			}
		}
	}
}
