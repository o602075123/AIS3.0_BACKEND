...
public class DocPackagesItemController extends BaseController {
	...
	@RequestMapping(value = "/deletePackagesItem")
	@ResponseBody
	@ApiOperation(value="删除收费项目",httpMethod="POST",notes="删除收费项目")
	public String deletePackagesItem(@RequestBody DocPackagesItem packagesItem){
		logger.info("----------------begin deletePackagesItem----------------");
		Map result = new HashMap();
		int flag = docPackagesItemService.deletePackagesItem(packagesItem);
		if(flag > 0){
			result.put("resultCode","1");
			result.put("resultMessage", "修改成功！");
		}
		logger.info("----------------end deletePackagesItem----------------");
