...
public class DocAnaesPacuObserveRecController extends BaseController{
    ...
    @RequestMapping(value = "/selectByPacuRecIdXYCD")
	@ResponseBody
	@ApiOperation(value="根据id查询复苏记录单",httpMethod="POST",notes="根据id查询复苏记录单")
	public String selectByPacuRecIdXYCD(@ApiParam(name="searchFormBean", value ="查询参数") @RequestBody SystemSearchFormBean searchFormBean) {
		...
		if ("05".equals(state) && null != anaesPacuRec.getBedId()) {
			if (null == anaesPacuRec.getEnterTime()) {
				docAnaesPacuRecService.startPacuRecEnterRoomTime(anaesPacuRec, date );
			}
...
//		}
		...
		if ("05".equals(state) && null != anaesPacuRec.getBedId()) {
			if (null == anaesPacuRec.getEnterTime()) {
				...
				List<SysCodeFormbean> values = basSysCodeService.searchSysCodeByGroupId("pacu_number", getBeid());
				if (null != values && values.size() > 0)
				{
				    pacuNumber = values.get(0).getCodeValue();
				    ...
				    basSysCodeService.updateValueByGroupId(s, "pacu_number");
				}
			}
...
//		} else {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/addPacuAnaesObserveHBGZB")
	@ResponseBody
	@ApiOperation(value="添加复苏室记录",httpMethod="POST",notes="添加复苏室记录")
	public String addPacuAnaesObserveHBGZB(@RequestBody DocAnaesPacuObserveRec record){
        logger.info("begin addPacuAnaesObserveHBGZB");
        ResponseValue resp = new ResponseValue();
        ValidatorBean validatorBean = beanValidator(record);
        if (!(validatorBean.isValidator()))
        {
...
//        }
        ...
        docAnaesPacuObserveRecService.savePacuAnaesObserve(record, resp);
        resp.put("pacuObsData", record);
        logger.info("end addPacuAnaesObserveHBGZB");
        return resp.getJsonStr();
	}
