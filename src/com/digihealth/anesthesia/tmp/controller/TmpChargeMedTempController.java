...
public class TmpChargeMedTempController extends BaseController{
	...
	public String searchTempInfoByTempId(@RequestBody Map map){
		...
		if(null!=map){
			...
			List<TmpChargePayTemp> chargeList = tmpPriChargeMedTempService.searchChargePayListById(chargeTempId,"1");//手术费
			List<TmpChargePayTemp> materialList = tmpPriChargeMedTempService.searchChargePayListById(chargeTempId,"2");//耗材费
			List<TmpChargePayTemp> anaesList = tmpPriChargeMedTempService.searchChargePayListById(chargeTempId,"3");//麻醉费
			...
			resp.put("chargeList", chargeList);
