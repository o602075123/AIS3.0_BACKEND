...
public class BasMonitorDisplayService extends BaseService{
	...
	public void changeMonitDisplay(MonitorDisplayChangeFormBean changeBean) {
		...
		if (StringUtils.isBlank(id)){
			newId = GenerateSequenceUtil.generateSequenceNo(GenerateSequenceUtil.getRoomId());
			remark = "新增";
		}else{
			newId = id;
	...
	@Transactional
    public void changeMonitDisplayQNZ(MonitorDisplayChangeFormBean changeBean) {
        // 第一步先修改 monitorDisplay
        String id = changeBean.getId();
        //Date time = changeBean.getTime();
        String regOptId = changeBean.getDocId();
	 ...
	 * @return
	 */
	public List<BasMonitorDisplay> getobsDataXYCD(MonitorDataFormBean bean,List<String> observeIds){
...
//		}
	...
	public Integer getobsDataTotalXYCD(MonitorDataFormBean bean,List<String> observeIds){
		String regOptId = bean.getRegOptId();
		Date time = bean.getInTime();
		String inTime = SysUtil.getTimeFormat(time);
		DocAnaesRecord anaesRecord = docAnaesRecordDao.searchAnaesRecordByRegOptId(regOptId);
	 ...
	 */
	@Transactional
	public List<BasMonitorDisplay> getobsDataNew4QNZ(MonitorDataFormBean formBean,List<String> observeIds){
		Date inTime = formBean.getInTime();
		Date sDate = formBean.getStartTime();
		String regOptId = formBean.getRegOptId();
		logger.info("sDate: "+sDate+"---inTime: "+inTime);
...
//				}
    ...
    public void updateEndTime(EndOperationFormBean formBean,ResponseValue res) {
        ...
        if(EvtAnaesEventService.OUT_ROOM.equals(code)){
            ...
            //将消息推送到手术室大屏   结束手术
            List<SysCodeFormbean> ls = basSysCodeDao.searchSysCodeByGroupIdAndCodeValue("leave_to", anaesevent.getLeaveTo(), getBeid());
            ...
            if(EvtAnaesEventService.CANCEL_OPER.equals(anaesevent.getCode())){
                //将取消的手术信息同步至his
            ...
            List<SysCodeFormbean> ls = basSysCodeDao.searchSysCodeByGroupIdAndCodeValue("leave_to", anaesRecord.getLeaveTo(), getBeid());
            String leaveToName = ls.size()>0?ls.get(0).getCodeName():"";
     ...
     */
    @Transactional
    public void updateEndTimeQNZ(EndOperationFormBean formBean,ResponseValue res) {
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        String docId = anaesevent.getDocId();
        String regOptId = formBean.getRegOptId();
        Integer code = anaesevent.getCode();
        ...
        if(EvtAnaesEventService.OUT_ROOM.equals(code)){
            ...
            if(StringUtils.isEmpty(isConnectionFlag) || "true".equals(isConnectionFlag)){
                try
                {
                    operListServiceQNZ.sendCancleRegOpt(regOptId, OperationState.POSTOPERATIVE);
                }
                catch (Exception e)
                {
                    logger.error("同步手术信息到his报错：  " + Exceptions.getStackTraceAsString(e));
                }
            }
            ...
            if(StringUtils.isEmpty(isConnectionFlag) || "true".equals(isConnectionFlag)){
                try
                {
                    operListServiceQNZ.sendCancleRegOpt(regOptId, OperationState.CANCEL);
                }
                catch (Exception e)
                {
                    logger.error("同步取消的手术信息到his报错：  " + Exceptions.getStackTraceAsString(e));
                }
            }
            List<SysCodeFormbean> ls = basSysCodeDao.searchSysCodeByGroupIdAndCodeValue("leave_to", anaesRecord.getLeaveTo(), getBeid());
            String leaveToName = ls.size()>0?ls.get(0).getCodeName():"";
     ...
     */
    @Transactional
    public void updateEndTimeXYCD(EndOperationFormBean formBean,ResponseValue res) {
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        String docId = anaesevent.getDocId();
        String regOptId = formBean.getRegOptId();
        Integer code = anaesevent.getCode();
...
//                }else{
	...
	@Transactional
	public void batchUpdateObsDat(DisplayDateChangeFormBean record) {
		
		List<BasMonitorDisplay> observeList = record.getObserveList();
		if(null!=observeList && observeList.size()>0){
			BaseInfoQuery baseQuery = new BaseInfoQuery();
			baseQuery.setRegOptId(record.getRegOptId());
			for (BasMonitorDisplay basMonitorDisplay : observeList) {
				baseQuery.setEventId(basMonitorDisplay.getObserveId());
				baseQuery.setBeid(getBeid());
				Float value = basMonitorDisplay.getValue();
				//从数据库中查询最大最小值
				BasAnaesMonitorConfigFormBean anaesMonitorConfigFormBean = basAnaesMonitorConfigDao.getAnaesMonitorConfigEventId(baseQuery);
				int state = -2;
				if(null != anaesMonitorConfigFormBean){
				...
				List<BasMonitorDisplay> monitorDispList = basMonitorDisplayDao.searchDisplayListByTimeAndId(record.getRegOptId(), basMonitorDisplay.getObserveId(), record.getStartTime(), record.getEndTime());
				for (BasMonitorDisplay basDisplayPo : monitorDispList) {
					basDisplayPo.setState(state);
					basDisplayPo.setValue(value);
					basMonitorDisplayDao.updateByPrimaryKey(basDisplayPo);
				}
			}
		}
	}
	...
	@Transactional
	public void getIntervalObsDataQNZ(IntervalDataFormBean bean) {
		List<Date> times = bean.getTimes();
		String regOptId = bean.getRegOptId();
		Integer freq = bean.getFreq();
		// 取beid
...
//		rescueevent.setState(anaesRecord.getState());
		...
		if(null != ctlBreathList && ctlBreathList.size()>0){
			for (int i = 0; i < ctlBreathList.size(); i++) {
                ...
                if(i == 0){
                    ...
                    List<SysCodeFormbean> scList = basSysCodeDao.searchSysCodeByGroupIdAndCodeName("breath_event", type + "", beid);
	...
	private List<SysCodeFormbean> getAnaesEventType(String code)
    {
        ...
        String beid = getBeid();
        anaesEventTypeList = basSysCodeDao.searchSysCodeByGroupIdAndCodeValue("anaes_event_type", code, beid);
        if (null == anaesEventTypeList || anaesEventTypeList.size() < 1)
        {
