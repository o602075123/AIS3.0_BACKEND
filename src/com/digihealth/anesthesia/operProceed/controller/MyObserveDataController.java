...
import com.digihealth.anesthesia.operProceed.formbean.DisplayDateChangeFormBean;
...
public class MyObserveDataController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("/updobsdatQNZ")
    @ResponseBody
    @ApiOperation(value = "修改点", httpMethod = "POST", notes = "修改点")
    public String updobsdatQNZ(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDisplayChangeFormBean changeBean) {
        logger.info("----------------start updobsdat------------------------");
        ResponseValue res = new ResponseValue();
        basMonitorDisplayService.changeMonitDisplayQNZ(changeBean);
        res.setResultCode("1");
        res.setResultMessage("操作成功");
        logger.info("------------------end updobsdat------------------------");
        return res.getJsonStr();
	 ...
	 * @return
	 */
	@RequestMapping("/batchUpdateObsDat")
	@ResponseBody
	@ApiOperation(value = "批量修改点", httpMethod = "POST", notes = "批量修改点")
	public String batchUpdateObsDat(@ApiParam(name = "params", value = "参数") @RequestBody DisplayDateChangeFormBean record) {
		logger.info("----------------start batchUpdateObsDat------------------------");
		ResponseValue res = new ResponseValue();
		basMonitorDisplayService.batchUpdateObsDat(record);
		res.setResultCode("1");
		res.setResultMessage("操作成功");
		logger.info("------------------end batchUpdateObsDat------------------------");
		return res.getJsonStr();
	}
	...
	@RequestMapping("/getobsDataXYCD")
	@ResponseBody
	@ApiOperation(value = "描点历史数据查询", httpMethod = "POST", notes = "描点历史数据查询")
	public Map getobsDataXYCD(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
		...
		List<BasMonitorDisplay> monitorList = basMonitorDisplayService.getobsDataXYCD(formBean, observeIds);
		...
		// 获取总数
		int total = basMonitorDisplayService.getobsDataTotalXYCD(formBean, observeIds);
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/getobsDataNewQNZ")
    @ResponseBody
    @ApiOperation(value = "获取新点", httpMethod = "POST", notes = "获取新点")
    public Map getobsDataNewQNZ(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
        logger.info("----------------start getobsDataNew------------------------");
        Map map = new HashMap();
        if (StringUtils.isBlank(formBean.getBeid())) {
            formBean.setBeid(getBeid());
        ...
        List<BasMonitorDisplay> monitorList = basMonitorDisplayService.getobsDataNew4QNZ(formBean, observeIds); // 增加特殊处理逻辑
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping("/getIntervalObsDataXYCD")
	@ResponseBody
	@ApiOperation(value = "页面再次打开，间隔点的处理", httpMethod = "POST", notes = "页面再次打开，间隔点的处理")
	public Map getIntervalObsDataXYCD(@ApiParam(name = "formbean", value = "参数")@RequestBody IntervalDataFormBean formbean) {
		...
		if (formbean != null) {
			...
			List<BasMonitorDisplay> monitorList = basMonitorDisplayService.getobsDataXYCD(mdFormBean, observeIds);
			...
			// 获取总数
			int total = basMonitorDisplayService.getobsDataTotalXYCD(mdFormBean, observeIds);
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping("/getIntervalObsDataQNZ")
	@ResponseBody
	@ApiOperation(value = "页面再次打开，间隔点的处理", httpMethod = "POST", notes = "页面再次打开，间隔点的处理")
	public Map getIntervalObsDataQNZ(@ApiParam(name = "formbean", value = "参数")@RequestBody IntervalDataFormBean formbean) {
		logger.info("----------------start getIntervalObsData------------------------");
		ResponseValue res = new ResponseValue();
		Map map = new HashMap();
		if (formbean != null) {
			basMonitorDisplayService.getIntervalObsDataQNZ(formbean); // 获取中间断的点
		...
		if (null != regOptId) {
			...
			if (null == regOpt) {
				...
				if (0 != emergency) {
					...
					if ((null != preVisit || null != preOperVisit || null != anaesPlan) && null != accede) {
					    String pv_processState = "";
					    if (null != preVisit)
					    {
					        pv_processState = preVisit.getProcessState();
					    }
					    if (null != preOperVisit && null != anaesPlan)
					    {
					        pv_processState = preOperVisit.getProcessState();
					    }
					    if (null != anaesPlan)
					    {
					        pv_processState = anaesPlan.getProcessState();
					    }
						...
						if (0 == regOpt.getIsLocalAnaes()) {
							if ("END".equals(pv_processState) && "END".equals(accede_processState)) {
								...
								res.setResultCode("10000000");
								res.setResultMessage("术前访视单或者麻醉计划未完成！");
		...
		if (StringUtils.isNotBlank(accessSource)) {
			...
			if ("start".equals(searchBean.getAccessSource())) {
				...
				if(flagInsert){
					...
					result.put("startOper", "如果为术中，继续发送，防止tomcat断掉后，导致不能再次连上采集服务");
					if("04".equals(opt.getState())){ //术中，继续发送，防止tomcat断掉后，导致不能再次连上采集服务
						CmdMsg msg = new CmdMsg();
						msg.setMsgType(MyConstants.OPERATION_STATUS_START);
						msg.setRegOptId(searchBean.getRegOptId());
						ResponseValue res = MessageProcess.process(msg);
            ...
            if(StringUtils.isBlank(anaesRecord.getOperRoomName()) && null != dispatch.getOperRoomId())
            {
                ...
                String operroomName = null == operroom ? null : operroom.getName();
                anaesRecord.setOperRoomName(operroomName);
                docAnaesRecordService.saveAnaesRecord(anaesRecord);
            }
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping("/startOperNHFE")
	@ResponseBody
	public String startOperNHFE(@RequestBody SearchFormBean searchBean){
		logger.info("----------------start startOper------------------------");
		...
		try {
			...
			if(StringUtils.isNotBlank(searchBean.getAccessSource())){
				...
				//当source传入start表示进入手术室，需要更新状态为术中，否则不更新当前状态
				if("start".equals(searchBean.getAccessSource())){
					Controller controller = controllerService.getControllerById(searchBean.getRegOptId());
					if(controller.getState().equals(OperationState.PREOPERATIVE)){
						controllerService.changeControllerState(searchBean.getRegOptId(), OperationState.SURGERY);
						flagInsert = true;
					}
				...
				//麻醉记录单
				anaesRecord = docAnaesRecordService.searchAnaesRecordByRegOptId(searchBean.getRegOptId());
				...
				searchBean.setDocId(anaesRecord.getAnaRecordId());
				// 手术信息表
				opt = basRegOptService.searchRegOptById(anaesRecord.getRegOptId());
				//获取排程信息
				BasDispatch dispatch = basDispatchService.getDispatchOper(anaesRecord.getRegOptId());
			...
			// 麻醉药事件明细   麻醉前用药
			searchBean.setType("02");
			List<RegOptOperMedicaleventFormBean> anaesMedEvtList = evtMedicaleventService.searchMedicaleventGroupByCodeListNHFE(searchBean);
			
			// 治疗药事件明细   用药 
			searchBean.setType("01");
			List<RegOptOperMedicaleventFormBean> treatMedEvtList = evtMedicaleventService.searchMedicaleventGroupByCodeListNHFE(searchBean);
			
			// 镇痛药事件明细   用药 
            searchBean.setType("03");
            List<RegOptOperMedicaleventFormBean> analgesicMedEvtList = evtMedicaleventService.searchMedicaleventGroupByCodeListNHFE(searchBean);
			...
			if ((null == anaesMonitorConfigFormBeanList || anaesMonitorConfigFormBeanList.size() <=  0) && null != showList && showList.size() > 0)
			{
			    ...
			    for (BasAnaesMonitorConfigFormBean anaesMonitorConfigFormBean : showList)
			    {
			        ...
			        BasMonitorConfigDefault monitorConfigDefault =
                        monitorConfigDefaultService.searchByEventName(anaesMonitorConfigFormBean.getEventName());
                    if (null != monitorConfigDefault)
                    {
                        //获取到启动设备对应的eventId，如果都没启动，则取默认值
                        String enableEventId = basMonitorConfigService.selectEventIdByEventName(anaesMonitorConfigFormBean,searchBean.getRegOptId());
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/startOperQNZ")
    @ResponseBody
    public String startOperQNZ(@RequestBody SearchFormBean searchBean){
        logger.info("----------------start startOper------------------------");
        ...
        try {
            ...
            // 麻醉药事件明细   麻醉前用药
            searchBean.setType("02");
            List<RegOptOperMedicaleventFormBean> anaesMedEvtList = evtMedicaleventService.searchMedicaleventGroupByCodeListQNZ(searchBean);
            ...
            // 镇痛药事件明细   用药 
            searchBean.setType("03");
            List<RegOptOperMedicaleventFormBean> analgesicMedEvtList = evtMedicaleventService.searchMedicaleventGroupByCodeListQNZ(searchBean);
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/startOperHBGZB")
    @ResponseBody
    @ApiOperation(value = "开始手术", httpMethod = "POST", notes = "开始手术")
    public String startOperHBGZB(@ApiParam(name = "params", value = "参数") @RequestBody SearchFormBean searchBean) {
        logger.info("----------------start startOper------------------------");
        ...
        List<RegOptOperIoeventFormBean> transfusioninIoeventList = evtInEventService.searchIoeventGroupByDefIdList(searchBean);
        //List<SearchOptOperIoevent> transfusioninIoeventList = evtInEventService.searchIoeventList(searchBean);

        searchBean.setSubType("2");// 输血
        List<RegOptOperIoeventFormBean> bloodinIoeventList = evtInEventService.searchIoeventGroupByDefIdList(searchBean);
        //List<SearchOptOperIoevent> bloodinIoeventList = evtInEventService.searchIoeventList(searchBean);
     ...
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping("/startOperXYCD")
    @ResponseBody
    public String startOperXYCD(@RequestBody SearchFormBean searchBean){
        logger.info("----------------start startOper------------------------");
    ...
    public String updateOuterTime(@RequestBody EndOperationFormBean formBean){
        logger.info("----------------start updateOuterTime------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        if (anaesevent != null)
        {
            // anaeseventService.saveAnaesevent(anaesevent);
        ...
        logger.info("------------------end updateOuterTime------------------------");
        return res.getJsonStr();
    }
     ...
     * 修改出室时间
     */
    @RequestMapping("/updateOuterTimeQNZ")
    @ResponseBody
    public String updateOuterTimeQNZ(@RequestBody EndOperationFormBean formBean){
        logger.info("----------------start updateOuterTime------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        if (anaesevent != null)
        {
            // anaeseventService.saveAnaesevent(anaesevent);
            basMonitorDisplayService.updateEndTimeQNZ(formBean, res);
            SearchFormBean searchBean = new SearchFormBean();
            searchBean.setDocId(anaesevent.getDocId());
            List<EvtAnaesEvent> resultList = new ArrayList<EvtAnaesEvent>();
            resultList = evtAnaesEventService.searchAnaeseventList(searchBean);
     ...
     * 修改出室时间（本溪局点）
     */
    @RequestMapping("/updateOuterTimeSYBX")
    @ResponseBody
    public String updateOuterTimeSYBX(@RequestBody EndOperationFormBean formBean){
        logger.info("----------------start updateOuterTime------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        if (anaesevent != null)
     ...
     * @return
     */
    @RequestMapping("/endOperationQNZ")
    @ResponseBody
    @ApiOperation(value = "正常结束手术", httpMethod = "POST", notes = "正常结束手术")
    public String endOperationQNZ(@ApiParam(name = "params", value = "参数") @RequestBody EndOperationFormBean formBean) {
        logger.info("----------------start endOperation------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        logger.info("endOperation----" + anaesevent);
    ...
    /**
     * 正常结束手术，发送采集数据模块，并发送采集服务(永兴局点)
     * 
     * @return
     */
    @RequestMapping("/endOperationYXRM")
    @ResponseBody
    @ApiOperation(value = "正常结束手术", httpMethod = "POST", notes = "正常结束手术")
    public String endOperationYXRM(@ApiParam(name = "params", value = "参数") @RequestBody EndOperationFormBean formBean) {
        logger.info("----------------start endOperationYXRM------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        logger.info("endOperation----" + anaesevent);
        String regOptId = formBean.getRegOptId();
        if (anaesevent != null) {
            basMonitorDisplayService.updateEndTimeYXRM(formBean,res);
            if (res.getResultCode().equals("1")) { // 只有成功了，才执行正常结束手术命令
                SearchFormBean searchBean = new SearchFormBean();
                searchBean.setDocId(anaesevent.getDocId());
                List<EvtAnaesEvent> resultList = evtAnaesEventService.searchAnaeseventList(searchBean);
        ...
        logger.info("------------------end endOperationYXRM------------------------");
        return res.getJsonStr();
    }
     ...
     * @return
     */
    @RequestMapping("/endOperationXYCD")
    @ResponseBody
    @ApiOperation(value = "正常结束手术", httpMethod = "POST", notes = "正常结束手术")
    public String endOperationXYCD(@ApiParam(name = "params", value = "参数") @RequestBody EndOperationFormBean formBean) {
        logger.info("----------------start endOperation------------------------");
        ResponseValue res = new ResponseValue();
        EvtAnaesEvent anaesevent = formBean.getAnaesevent();
        logger.info("endOperation----" + anaesevent);
        ...
        if (anaesevent != null) {
            ...
            basMonitorDisplayService.updateEndTimeXYCD(formBean,res);
	 ...
	 * @return
	 */
	@RequestMapping("/getmonDataXYCD")
	@ResponseBody
	@ApiOperation(value = "获取监测项分页历史数据", httpMethod = "POST", notes = "获取监测项分页历史数据")
	public String getmonDataXYCD(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
		...
		if (null != monitorList && monitorList.size() > 0) {
			for (BasMonitorDisplay md : monitorList) {
				...
				if (!map.containsKey(time)) {
					...
					mds = map.get(time);
					//前端为了展示需要，需要将O2放在第一个位置
					if (O2_EVENT_ID.equals(md.getObserveId())) 
					{
					    mds.add(0, md);
					}
					else
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping("/getPrintObserveDataNHFE")
	@ResponseBody
	@ApiOperation(value = "分组获取打印数据", httpMethod = "POST", notes = "分组获取打印数据")
	public Map getPrintObserveDataNHFE(@ApiParam(name = "params", value = "参数") @RequestBody MonitorDataFormBean formBean) {
		logger.info("----------------start getPrintObserveData------------------------");
		Map map = new HashMap();
		List<MonitorDataPage> mdPageList = new ArrayList<MonitorDataPage>();
			...
			if (no == 0) {
				...
				if (null != anaesLists && anaesLists.size() > 0) {
					for (BasAnaesMonitorConfigFormBean bean : anaesLists) {
						...
						String icon = bean.getIconId();// 对应颜色
						String units = bean.getUnits(); // 默认单位
						String svg = basIconSvgService.getPathByIcon(icon,this.getBeid());
						Float max = bean.getMax();  //max
	                    Float min = bean.getMin();  //min
	                    
						if (observeId.equals("30008") || observeId.equals("30010") || observeId.equals("31008") || observeId.equals("31010")|| observeId.equals("30016") || observeId.equals("31016")) { // 如果是温度，则y轴为1
							seriesMap.put(observeId, new SeriesData(observeId,observeName, "line", new ArrayList<SeriesDataObj>(), icon, svg, 1, 
									MyObserveDataController.SYMBOL_PRINTDATA, color, units,max,min));
							yAxis.get(1).setMax(max);
		                    yAxis.get(1).setMin(min);
		                    yAxis.get(1).setName(units);
						} else {
							seriesMap.put(observeId, new SeriesData(observeId,observeName, "line", new ArrayList<SeriesDataObj>(), icon,svg, 0, 
									MyObserveDataController.SYMBOL_PRINTDATA, color, units,max,min));
							yAxis.get(0).setMax(max);
		                    yAxis.get(0).setMin(min);
		                    yAxis.get(0).setName(units);
						}
					}
				}
                        ...
                        if (null == operTime) {
                            ...
                            if (time == 0) {
                                ...
                                if (null != curMd) {
                                    ...
                                    if (operFreq.intValue() != intervalTime.intValue()) {
                                        long myTime = ((curTime.getTime() + intervalTime * 1000) - inTime.getTime()) / 1000;
                                        if (myTime > 0) { // 1、如果最近点+intervalTime大于intime，则offset = (curTime+interval_time)-inTime
                                            map.put("offset", myTime);
                                        } else if (myTime == 0) {
                                            map.put("offset", 0);
                                        } else { // curTime+intervalTime 必须大于 inTime ，不然数据有误
	 ...
	 * 当前用户下的手术（麻醉医生和巡回护士）
	 */
	@RequestMapping("/getCurUserOperHBGZB")
	@ResponseBody
	@ApiOperation(value = "当前用户下的手术", httpMethod = "POST", notes = "当前用户下的手术")
	public String getCurUserOperHBGZB(@ApiParam(name = "params", value = "参数") @RequestBody JSONObject json) {
		logger.info("------------------start getCurUserOperHBGZB------------------------");
		ResponseValue res = new ResponseValue();
		String userId = json.get("userId").toString();
		List<SearchRegOptByRoomIdAndOperDateAndStateFormBean> resultList = basRegOptService.searchCurUserRegOptHBGZB(Global.getConfig("roomId").toString(), DateUtils.getDate(), userId);
		res.put("resultList", resultList);
		String operRoomName = resultList.size() > 0 ? resultList.get(0).getOperRoomName() : basOperroomService.queryRoomListById(Global.getConfig("roomId").toString()).getName();
		res.put("operRoomName", operRoomName);
		res.put("roomId", Global.getConfig("roomId").toString());
		logger.info("------------------end getCurUserOperHBGZB------------------------");
		return res.getJsonStr();
	}

	/**
	 * -- 当前麻醉医生或巡回护士下面的手术（包含当前手术室下术中的手术和当前登录用户下的术前的手术）
	 * 麻醉医生只显示全麻的术前手术
	 * 巡回护士显示全麻和局麻的术前手术；
	 * 
	 * 当前用户下的手术（麻醉医生和巡回护士）
	 */
	@RequestMapping("/getCurUserOperNHFE")
	@ResponseBody
	public String getCurUserOperNHFE(@RequestBody JSONObject json){
		logger.info("------------------start getCurUserOper------------------------");
		ResponseValue res = new ResponseValue();
		try {
			String userId = json.get("userId").toString();
			BasUser user = basUserService.searchUserById(userId,this.getBeid());
			List<SearchRegOptByRoomIdAndOperDateAndStateFormBean> resultList = basRegOptService
					.searchCurUserRegOptNHFE(Global.getConfig("roomId").toString(), DateUtils.getDate(), userId);
			res.put("resultList", resultList);
			String operRoomName = resultList.size()>0?resultList.get(0).getOperRoomName():basOperroomService.queryRoomListById(Global.getConfig("roomId").toString()).getName();
			res.put("operRoomName", operRoomName);
			res.put("roomId", Global.getConfig("roomId").toString());
		} catch (Exception e) {
			logger.error("getCurUserOper-----"+Exceptions.getStackTraceAsString(e));
			res.setResultCode("10000000");
	...
	@RequestMapping("/getPupilDataXYCD")
	@ResponseBody
	@ApiOperation(value = "getPupilDataXYCD", httpMethod = "POST", notes = "getPupilData")
	public String getPupilDataXYCD(@ApiParam(name = "formBean", value = "查询参数") @RequestBody MonitorDataFormBean formBean) {
    ...
    public String searchIOAmoutDetail(@RequestBody SearchFormBean searchBean)
    {
        ...
        //失血量
        Integer bleeding = evtEgressService.getEgressCountValueByIoDefName("出血量", docId);
        res.put("bleeding", bleeding);
        //尿量
        Integer urine = evtEgressService.getEgressCountValueByIoDefName("尿量", docId);
        res.put("urine", urine);
        //输液
        Integer infusion = evtInEventService.getIoeventCountValueByIoDef(docId, null, "1");
        res.put("infusion", infusion);
        //输血
        Integer blood = evtInEventService.getIoeventCountValueByIoDef(docId, null, "2");
    ...
    @ResponseBody
    public String isNeedAnaesDoctor(@RequestBody Map map)
    {
