...
public class BasRegOptService extends BaseService {
	...
	public List<SearchRegOptByRoomIdAndOperDateAndStateFormBean> searchCurUserRegOptNHFE(String roomId, String operDate, String userId) {
	    String roleType = null;
        String beid = getBeid();
        BasUser user = basUserDao.selectByPrimaryKey(userId, beid);
        if (null != user) {
            roleType = user.getRoleType();
        }
		return basRegOptDao.searchCurUserRegOptNHFE(roomId, operDate, userId, beid, roleType);
	}

	public List<SearchRegOptByRoomIdAndOperDateAndStateFormBean> searchCurUserRegOptHBGZB(String roomId, String operDate, String userId) {
		String roleType = null;
		String beid = getBeid();
		BasUser user = basUserDao.selectByPrimaryKey(userId, beid);
		if (null != user) {
		...
		return basRegOptDao.searchCurUserRegOptHBGZB(roomId, operDate, userId, beid, roleType);
	}
	...
	@Transactional
	public String updateRegOptInfo(BasRegOpt regOpt, BasDispatch dispatch) {
		if (regOpt != null && (!StringUtils.isEmpty(regOpt.getRegOptId()))) {
			...
			basRegOptDao.updateRegOptBaseInfo(regOpt); 
		}
		...
		if (null != regOpt && "03".equals(regOpt.getState()) && 0 == regOpt.getIsLocalAnaes()) {
			SearchFormBean searchBean = new SearchFormBean();
	...
	@Transactional
    public String updateRegOptInfoQNZ(BasRegOpt regOpt, BasDispatch dispatch) {
	    String beid = getBeid();
	    if (regOpt != null && (!StringUtils.isEmpty(regOpt.getRegOptId()))) {
        ...
        if (null != regOpt && 0 != regOpt.getEmergency() && "02".equals(regOpt.getState()))
       {
        ...
        if (null != regOpt && 
            ("03".equals(regOpt.getState()) 
                || (0 != regOpt.getEmergency() && "02".equals(regOpt.getState()))) 
            ...
            if(StringUtils.isEmpty(isConnectionFlag) || "true".equals(isConnectionFlag)){
                ...
                try
                {
                    ...
                    operListServiceQNZ.sendDispatchInfo(dispatch, regOpt);
                }
                catch (Exception e)
                {
     ...
     */
    @Transactional
    public void cancleRegOptQNZ(CancleRegOptFormBean cancleRegOptFormBean, ResponseValue resp) {
        BasRegOpt regOpt = basRegOptDao.searchRegOptById(cancleRegOptFormBean.getRegOptId());
        regOpt.setReasons(cancleRegOptFormBean.getReasons());
        basRegOptDao.updateByPrimaryKeySelective(regOpt);
        Controller controller = controllerDao.getControllerById(cancleRegOptFormBean.getRegOptId());
        ...
        if (OperationState.NOT_REVIEWED.equals(controller.getState()) || OperationState.NO_SCHEDULING.equals(controller.getState()) || OperationState.PREOPERATIVE.equals(controller.getState())) {
            ...
            String isConnectionFlag = Global.getConfig("isConnectionHis").trim();
            if(StringUtils.isEmpty(isConnectionFlag) || "true".equals(isConnectionFlag)){
                logger.info("===============================发送取消手术请求到his===========================================");
                try
                {
                    OperListServiceQNZ operListService = SpringContextHolder.getBean(OperListServiceQNZ.class);
                    operListService.sendCancleRegOpt(regOpt.getRegOptId(), OperationState.CANCEL);
                }
                catch (Exception e)
                {
                    logger.error("同步手术取消信息到his报错：  " + Exceptions.getStackTraceAsString(e));
                }
            }
            resp.setResultCode("1");
            resp.setResultMessage("取消手术成功!");
	...
	@Transactional
    public void activeRegOptQNZ(CancleRegOptFormBean acticRegOptFormBean, ResponseValue resp) {
        BasRegOpt regOpt = basRegOptDao.searchRegOptById(acticRegOptFormBean.getRegOptId());
        if (!OperationState.CANCEL.equals(regOpt.getState()) || OperationState.SURGERY.equals(regOpt.getPreviousState())) {
            resp.setResultCode("30000001");
            resp.setResultMessage("当前手术不允许激活!");
        } else {
            regOpt.setReasons(acticRegOptFormBean.getReasons());
            basRegOptDao.updateByPrimaryKeySelective(regOpt);
            Controller controller = controllerDao.getControllerById(acticRegOptFormBean.getRegOptId());
	...
	public int autoArchiveDocuments() {
		...
		List<SearchMyOperationFormBean> list = basRegOptDao.searchNoArchiveRegOpt(time, getBeid());
