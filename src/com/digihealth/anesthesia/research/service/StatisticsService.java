...
import com.digihealth.anesthesia.research.formbean.SearchAnesDetailFormbean;
...
public class StatisticsService extends BaseService{
	...
	public String updateMedIoCharge(MedIoChargeFormBean medIoChargeFormBean){
		...
		List<DocPackagesItem> anaesList = medIoChargeFormBean.getAnaesList();
			...
			for(int i = 0 ; i < medList.size();i++){
				if(StringUtils.isNotBlank(medList.get(i).getEbId())){
					...
					if(eventBilling.getCode().equals("201709301805128404")){
		        		...
		        		eventBilling.setDosageTotalAmount(null == eventBilling.getPackageDosageAmount() ? 0 : dosAmt.multiply(new BigDecimal(eventBilling.getPackageDosageAmount())).floatValue());
		        	}
		...
		if(anaesList!=null &&anaesList.size()>0){
            for(int i = 0 ; i < anaesList.size();i++){
                if(StringUtils.isNotBlank(anaesList.get(i).getPkItId())){
                    DocPackagesItem packagesItem = docPackagesItemDao.searchPackagesItemById(anaesList.get(i).getPkItId());
                    packagesItem.setDiscount(anaesList.get(i).getDiscount());
                    packagesItem.setIsCharge(anaesList.get(i).getIsCharge());
                    packagesItem.setChargeAmount(anaesList.get(i).getChargeAmount());
                    docPackagesItemDao.update(packagesItem);
                }
            }
        }
	...
	public List<SearchAnesTypeFormBean> searchAnaesMethodByDept(
		...
		SearchAnesTypeFormBean otherVo = statisticsDao.countAnesNumByOtherAnesType(searchFormBean);// 其他麻醉方式
     ...
     * @return
     */
    public List<SearchAnesTypeFormBean> searchAnaesMethodByDeptQNZ(SearchFormBean searchFormBean) {
        List<SearchAnesTypeFormBean> resultList = new ArrayList<SearchAnesTypeFormBean>();
        SearchAnesTypeFormBean sum = new SearchAnesTypeFormBean();
        sum.setTotalNum(0);
        sum.setTotalTime(0.0f);

        List<SysCodeFormbean> anaesTypeList = basSysCodeDao.searchSysCodeByGroupId("anes_type", getBeid());
        if (null != anaesTypeList && anaesTypeList.size() > 0)
        {
            for (SysCodeFormbean bean : anaesTypeList)
            {
                searchFormBean.setType(bean.getCodeValue());
                SearchAnesTypeFormBean formBean = new SearchAnesTypeFormBean();
                if ("3".equals(bean.getCodeValue()))
                {
                    formBean = statisticsDao.countAnesNumByCombineAnaes(searchFormBean);
                }
                else if ("4".equals(bean.getCodeValue()))
                {
                    formBean = statisticsDao.countAnesNumByLocalAnaes(searchFormBean);
                }
                else
                {
                    formBean = statisticsDao.countAnesNumByAnesType(searchFormBean);
                }
                formBean.setAnesType(bean.getCodeName());
                resultList.add(formBean);
                sum.setTotalNum(sum.getTotalNum() + formBean.getTotalNum());
                sum.setTotalTime(sum.getTotalTime() + formBean.getTotalTime());
            }
            
        }
        BigDecimal bd = new BigDecimal((sum.getTotalTime() / sum.getTotalNum()));
        bd = bd.setScale(0, 4);
        sum.setAvgTime(bd.floatValue());
        sum.setAnesType("总计");
        resultList.add(sum);
        return resultList;
    }
	...
	public List<AnaesDocObserveTimeCount> searchDeathCountByDept(SystemSearchFormBean searchFormBean){
        if(StringUtils.isBlank(searchFormBean.getBeid())) {
            searchFormBean.setBeid(getBeid());
        }
        List<Filter> filters = searchFormBean.getFilters();
	...
	public List<StaticDeptCountByAnalgesic> searchDeptCountByAnalgesicQNZ(SearchFormBean searchBean){
        return statisticsDao.searchDeptCountByAnalgesicQNZ(searchBean);
    }
    ...
    public Integer searchOperByAnalgesicQNZ(SearchFormBean searchBean) {
        if (StringUtils.isBlank(searchBean.getBeid())) {
            searchBean.setBeid(getBeid());
        }
        return statisticsDao.searchOperByAnalgesicQNZ(searchBean);
    }
     ...
     * @return
     */
    public List<WorkingTimeFormBean> searchNhNurseQmWorkingTimeHBGZB(String startTime,
			 String endTime,String loginName, String beid){
    	return statisticsDao.searchLgNurseQmWorkingTimeHBGZB(startTime, endTime, loginName, beid);
    }
     ...
     * @return
     */
    public List<WorkingTimeFormBean> searchNhNurseJmWorkingTimeHBGZB(String startTime,
			 String endTime,String loginName, String beid){
    	return statisticsDao.searchLgNurseJmWorkingTimeHBGZB(startTime, endTime, loginName, beid);
    }
	...
	public List<GetPharmacyReportFormBean> getPharmacyReport(SearchFormBean searchFormBean){
        return statisticsDao.getPharmacyReport(searchFormBean);
    }
	
	public int searchOperNoStartAt9(SearchFormBean searchFormBean){
        return statisticsDao.searchOperNoStartAt9(searchFormBean);
    }
	
	public int searchOperNoEndAt18(SearchFormBean searchFormBean){
        return statisticsDao.searchOperNoEndAt18(searchFormBean);
    }
	
	public List<SearchAnesTypeFormBean> searchAnesDetailByCond(SearchFormBean searchFormBean)
	{
	    List<SearchAnesTypeFormBean> resultList = new ArrayList<SearchAnesTypeFormBean>();
        ...
        if (null != anaesTypeList && anaesTypeList.size() > 0)
        {
            ...
            {
                SearchAnesTypeFormBean formBean = new SearchAnesTypeFormBean();
                List<SearchAnesDetailFormbean> detailList = new ArrayList<SearchAnesDetailFormbean>();
                List<SearchAnesDetailFormbean> specialDetailList = new ArrayList<SearchAnesDetailFormbean>();
                String anesType = bean.getCodeValue();
                String beid = searchFormBean.getBeid();
                String startTime = searchFormBean.getStartTime();
                String endTime = searchFormBean.getEndTime();
                //插管全麻
                if ("1".equals(anesType))
                {
                    detailList = statisticsDao.searchAnesDetailByCond("5", beid, anesType, null, startTime, endTime);
                    formBean.setAnesType(anesType);
                    formBean.setAnesTypeName(bean.getCodeName());
                }
                //非插管全麻
                else if ("2".equals(anesType))
                {
                    detailList = statisticsDao.searchAnesDetailByCond("6", beid, anesType, null, startTime, endTime);
                    formBean.setAnesType(anesType);
                    formBean.setAnesTypeName(bean.getCodeName());
                } 
                //局麻
                else if ("12".equals(anesType))
                {
                    detailList = statisticsDao.searchLocalAnesDetail(beid, anesType, startTime, endTime);
                    formBean.setAnesType(anesType);
                    formBean.setAnesTypeName(bean.getCodeName());
                }
                else
                {
                    detailList = statisticsDao.searchAnesDetailByCond("1", beid, anesType, null, startTime, endTime);
                    formBean.setAnesType(anesType);
                    formBean.setAnesTypeName(bean.getCodeName());
                }
                //全麻+神经阻滞统计 需要统计anesType为10的单一麻醉方法和全麻加神经阻滞的麻醉方法
                if ("10".equals(anesType))
                {
                    specialDetailList = statisticsDao.searchAnesDetailByCond("3", beid, "11", "1", startTime, endTime);
                    detailList.addAll(specialDetailList);
                }
                //其他麻醉 统计anesType为13的单一麻醉方法 + 非全麻加神经阻滞的麻醉方法组合 + 麻醉方法大于或等于三个的麻醉组合
                else if ("13".equals(anesType))
                {
                    specialDetailList = statisticsDao.searchAnesDetailByCond("4", beid, "11", "1", startTime, endTime);
                    detailList.addAll(specialDetailList);
                    specialDetailList = statisticsDao.searchAnesDetailByCond("2", beid, null, null, startTime, endTime);
                    detailList.addAll(specialDetailList);
                }
                
                formBean.setAnesDetailList(detailList);
                formBean.setTotalNum(detailList.size());
                resultList.add(formBean);
            }
        }
	    return resultList;
