...
import com.digihealth.anesthesia.research.formbean.GetPharmacyReportFormBean;
...
public class StatisticsController extends BaseController {
	...
	public Map statisAnaesOptNum(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
		...
		{
		    iyAxis.setMax((compareMap.get("max") / 5 + 1) * 5);
		}
		if (null != compareMap.get("min"))
        {
		    iyAxis.setMin(compareMap.get("min") < 0 ? compareMap.get("min") : 0);
        }
    ...
    public String searchAnaesMethodByDept(@ApiParam(name="searchFormBean", value ="统计查询参数") @RequestBody SearchFormBean searchFormBean) {
        ...
        yAxis.setMin((float)num);
        yAxis.setMax((float)max);
     ...
     * @return
     */
    @RequestMapping(value = "/countAnaesDeptByMethodTypeQNZ")
    @ResponseBody
    @ApiOperation(value="麻醉科麻醉例数统计",httpMethod="POST",notes="麻醉科麻醉例数统计")
    public String searchAnaesMethodByDeptQNZ(@ApiParam(name="searchFormBean", value ="统计查询参数") @RequestBody SearchFormBean searchFormBean) {
        logger.info("begin countAnaesDeptByMethodType");
        ResponseValue resp = new ResponseValue();
        ...
        List<SysCodeFormbean> anaesTypeList = basSysCodeService.searchSysCodeByGroupId("anes_type", getBeid());
        if (null != anaesTypeList && anaesTypeList.size() > 0)
        {
            for (SysCodeFormbean bean : anaesTypeList)
            {
                legendData.add(bean.getCodeName());
            }
        }
...
//        legendData.add("其他麻醉方式");
        ...
        series.setName("麻醉类型");
        series.setType("bar");
        List<SearchAnesTypeFormBean> list = statisticsService.searchAnaesMethodByDeptQNZ(searchFormBean);
        List<Float> chartData = new ArrayList<Float>();
        Integer max = 1,min = 0, num = 0;
        for (SearchAnesTypeFormBean searchAnesTypeFormBean : list) {
            if(searchAnesTypeFormBean.getTotalNum() > max) {
    ...
    public String searchWorkingTime1(@ApiParam(name="searchConditionFormBean", value ="查询参数对象") 
        ...
        Map<String,Object> anaesRecordMap = new HashMap<String,Object>();
        BasUser user = basUserService.selectByUsername(searchConditionFormBean.getLoginName(), getBeid());
        if (user != null) {
        	if(user.getRoleType().equals("ANAES_DOCTOR") || user.getRoleType().equals("ANAES_DIRECTOR")){
        		...
        		if (anaesRecordList != null && anaesRecordList.size() > 0) 
        		{
        			for (int i = 0; i < anaesRecordList.size(); i++)
        			{
        				HomeAnaesDoctorWorkingTimeFormBean anaesRecord = anaesRecordList.get(i);
        				if(null != anaesRecord)
        				{
        					String outOperTime = anaesRecord.getOutOperRoomTime();
            				String intOperTime = anaesRecord.getAnaesStartTime();
            				String anesthetistId = anaesRecord.getAnesthetistId();
            				long workingtime = 0;
            				if(StringUtils.isNotBlank(outOperTime) && StringUtils.isNotBlank(intOperTime))
            				{
            					workingtime = (DateUtils.getParseTime(outOperTime).getTime() - DateUtils.getParseTime(intOperTime).getTime())/1000/60 ;
            				...
            				//此块统计排班里面的主麻的工作量
            				if((user.getUserName()+"").equals(anesthetistId))
            				{
            					WorkingTimeFormBean old = (timeMap
            							.get(anaesRecord.getYear() + "年"+ anaesRecord.getMonth() + "月"));
            					long l = Long.parseLong(old.getTime()) + workingtime;
            					old.setTime(l+"");
            					timeMap.put(anaesRecord.getYear() + "年"+ anaesRecord.getMonth() + "月", old);
            				}
        				}
        			}
        		}
        			...
        			if(i + 1 < shiftChanges.length){
        				if(shiftChanges[i].getRegOptId().equals(shiftChanges[i+1].getRegOptId())){
        					...
        					if((user.getUserName()+"").equals(shiftChanges[i].getShiftChangePeopleId())){
        						...
        						{
        							long tmp = 0;
        							String outOperTime = workingbean.getOutOperRoomTime();
                    				String intOperTime = workingbean.getAnaesStartTime();
                    				if(StringUtils.isNotBlank(outOperTime) && StringUtils.isNotBlank(intOperTime))
            						{
            							tmp = (DateUtils.getParseTime(outOperTime).getTime() - DateUtils.getParseTime(intOperTime).getTime())/1000/60 ;
            						}
                    				WorkingTimeFormBean old = (timeMap
            								.get(shiftChanges[i].getYear() + "年"+ shiftChanges[i].getMonth() + "月"));
            						long l = Long.parseLong(old.getTime()) + tmp;
        	...
        	}else if(user.getRoleType().equals("NURSE") || user.getRoleType().equals("HEAD_NURSE")){
        		//全麻
        		resultList = statisticsService
        				.searchNhNurseQmWorkingTime(startTime, endTime, searchConditionFormBean.getLoginName(), beid);
        		//局麻
        		resultList.addAll(statisticsService
        				.searchNhNurseJmWorkingTime(startTime, endTime, searchConditionFormBean.getLoginName(), beid));
    ...
    /**
     * 
         * @discription 首页工作量计算 湖北葛洲坝
         * @author chengwang       
         * @created 2016年11月18日 下午2:23:11     
         * @param searchConditionFormBean
         * @return
     */
    @RequestMapping(value = "/searchWorkingTimeHBGZB")
    @ResponseBody
	@ApiOperation(value="手术时长统计",httpMethod="POST",notes="手术时长统计")
    public String searchWorkingTimeHBGZB(@ApiParam(name="searchConditionFormBean", value ="查询参数对象") 
            @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchWorkingTime");
        ...
        if (user != null) {
        	if(user.getRoleType().equals("ANAES_DOCTOR") || user.getRoleType().equals("ANAES_DIRECTOR")){
        		...
        		//全麻
        		resultList = statisticsService
        				.searchNhNurseQmWorkingTimeHBGZB(startTime, endTime, searchConditionFormBean.getLoginName(), beid);
        		//局麻
        		resultList.addAll(statisticsService
        				.searchNhNurseJmWorkingTimeHBGZB(startTime, endTime, searchConditionFormBean.getLoginName(), beid));
	...
	@ResponseBody
	public String searchMedIoCharge(@RequestBody SystemSearchFormBean systemSearchFormBean) {
		logger.info("begin searchMedIoCharge");
		Map map = new HashMap();
		
        // 手术费用
		BigDecimal chargeShouldSum = new BigDecimal(0);
        List<DocPackagesItem> charge = docPackagesItemService.queryPackagesItemList(
                systemSearchFormBean, 1 ,chargeShouldSum);
        
        // 材料费用
        BigDecimal materialShouldSum = new BigDecimal(0);
        List<DocPackagesItem> materialList = docPackagesItemService.queryPackagesItemList(
                systemSearchFormBean, 2 ,materialShouldSum);
        
        //麻醉费用
        BigDecimal anaesShouldSum = new BigDecimal(0);
        List<DocPackagesItem> anaesList = docPackagesItemService.queryPackagesItemList(
                systemSearchFormBean, 3 ,anaesShouldSum);
        
		// 药品
        BigDecimal medShouldSum = new BigDecimal(0);
		...
		if(null!=medList && medList.size()>0){
            for (EventBillingFormBean eventBillingFormBean : medList) {
            	/*if(eventBillingFormBean.getCode().equals("201709301805128404")){
            		float priceMinPackage = (float)(Math.round((eventBillingFormBean.getPriceMinPackage()/eventBillingFormBean.getPackageDosageAmount())*100))/100;
            		eventBillingFormBean.setPriceMinPackage(priceMinPackage);
            		eventBillingFormBean.setPackageTotalAmount(eventBillingFormBean.getDosageTotalAmount());
            		eventBillingFormBean.setShouldCost((float)(Math.round((eventBillingFormBean.getPackageTotalAmount()*priceMinPackage)*100))/100);
            	}*/
                medShouldSum.add(new BigDecimal(eventBillingFormBean.getShouldCost()));
            }
        }
		
		/*BigDecimal ioShouldSum = new BigDecimal(0);
		// 入量
		...
		if(null!=ioList && ioList.size()>0){
            for (EventBillingFormBean eventBillingFormBean2 : ioList) {
                ioShouldSum.add(new BigDecimal(eventBillingFormBean2.getShouldCost()));
            }
        }
		*/
		...
		String regOptId = "";
		for (Filter fi : fList) {
			if("regOptId".equals(fi.getField())){
        ...
        if(0 == regOpt.getIsLocalAnaes()){  //全麻
		...
		String instrnurse = (StringUtils.isEmpty(instrnurse1)?"":instrnurse1) + (StringUtils.isEmpty(instrnurse2)?"":","+instrnurse2); 
            ...
            for(int i = 0;i<filters.size();i++){
                if(!StringUtils.isEmpty(filters.get(i).getValue().toString())){
                    if("costType".equals(filters.get(i).getField().toString())){
                        costType = filters.get(i).getValue();
                        break;
                    }
                }
            }
        }
        map.put("materialShouldSum", materialShouldSum);
        map.put("chargeShouldSum", chargeShouldSum);
        map.put("medShouldSum", medShouldSum);
        //map.put("anaesShouldSum", anaesShouldSum);
        //map.put("ioShouldSum", ioShouldSum);
        map.put("shouldCostTotal", docEventBillingService.queryShouldCostByRegOptId(regOptId, costType));
    ...
    @RequestMapping(value = "/searchDeptOperatCountByOptlev")
    @ResponseBody
    public String searchDeptOperatCountByOptlev(
            @RequestBody SearchFormBean searchFormBean) {
        logger.info("begin searchDeptOperatCountByOptlev");
        ResponseValue resValue = new ResponseValue();
        List<SearchDeptOperatCountByOptlev> list = statisticsService.searchDeptOperatCountByOptlev(searchFormBean);
        ...
        List<Float> fstData = new ArrayList<Float>();
        List<Float> secData = new ArrayList<Float>();
        List<Float> thdData = new ArrayList<Float>();
        List<Float> fouData = new ArrayList<Float>();
        
        List<SysCodeFormbean> fieldList = basSysCodeService.searchSysCodeByGroupId("operat_level", null);
        for (SearchDeptOperatCountByOptlev searchDeptOperatCountByOptlev : list) {
            
            sdata.add(searchDeptOperatCountByOptlev.getDeptName());
            
            fstData.add((float)searchDeptOperatCountByOptlev.getFst());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByOptlev.getFst());
            
            secData.add((float)searchDeptOperatCountByOptlev.getSec());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByOptlev.getSec());
            
            thdData.add((float)searchDeptOperatCountByOptlev.getThd());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByOptlev.getThd());
            
            fouData.add((float)searchDeptOperatCountByOptlev.getFou());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByOptlev.getFou());
        }
        List<Map> columnDefs = new ArrayList<Map>();
        
        for (SysCodeFormbean sysCodeFormbean : fieldList) {
            Map map = new HashMap();
            
            Series series = new Series();
            series.setName(sysCodeFormbean.getCodeName());
            String field = "";
            if("1".equals(sysCodeFormbean.getCodeValue())){
                field = "fst";
                series.setData(fstData);
            }
            if("2".equals(sysCodeFormbean.getCodeValue())){
                field = "sec";
                series.setData(secData);
            }
            if("3".equals(sysCodeFormbean.getCodeValue())){
                field = "thd";
                series.setData(thdData);
            }
            if("4".equals(sysCodeFormbean.getCodeValue())){
                field = "fou";
                series.setData(fouData);
            }
            map.put("field", field);
            map.put("name", sysCodeFormbean.getCodeName());
            
            columnDefs.add(map);
            legendData.add(sysCodeFormbean.getCodeName());
        ...
        tmap.put("field", "deptName");
        tmap.put("name", "科室");
        columnDefs.add(0,tmap);
        
        tmap = new HashMap();
        tmap.put("field", "total");
        tmap.put("name", "总计");
        columnDefs.add(tmap);
        ...
        resValue.put("columnDefs", columnDefs);
        resValue.put("xAxis", xAis);
        resValue.put("yAxis", iyAxis);
        resValue.put("series", seriesList);
        resValue.put("legend", legend);
        resValue.put("tableList", list);
        logger.info("end searchDeptOperatCountByOptlev");
        return resValue.getJsonStr();
    }
     ...
     * @return
     */
    @RequestMapping(value = "/searchDeptOperatCountByAsalev")
    @ResponseBody
    public String searchDeptOperatCountByAsalev(
            @RequestBody SearchFormBean searchFormBean) {
        logger.info("begin searchDeptOperatCountByAsalev");
        ResponseValue resValue = new ResponseValue();
        List<SearchDeptOperatCountByAsalev> list = statisticsService.searchDeptOperatCountByAsalev(searchFormBean);
        String[] time = new String[list.size()];
        List<SysCodeFormbean> fieldList = basSysCodeService.searchSysCodeByGroupId("asa_level", null);
        ...
        List<Float> thdData = new ArrayList<Float>();
        List<Float> fouData = new ArrayList<Float>();
        List<Float> fifData = new ArrayList<Float>();
        //List<Float> sixData = new ArrayList<Float>();

        for (SearchDeptOperatCountByAsalev searchDeptOperatCountByAsalev : list) {
            
            sdata.add(searchDeptOperatCountByAsalev.getDeptName());
            
            fstData.add((float)searchDeptOperatCountByAsalev.getFst());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByAsalev.getFst());
            
            secData.add((float)searchDeptOperatCountByAsalev.getSec());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByAsalev.getSec());
            
            thdData.add((float)searchDeptOperatCountByAsalev.getThd());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByAsalev.getThd());
            
            fouData.add((float)searchDeptOperatCountByAsalev.getFou());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByAsalev.getFou());
            
            fifData.add((float)searchDeptOperatCountByAsalev.getFif());
            setMaxAndMin(compareMap, (float)searchDeptOperatCountByAsalev.getFif());
            
//            sixData.add((float)searchDeptOperatCountByAsalev.getSix());
//            setMaxAndMin(compareMap, (float)searchDeptOperatCountByAsalev.getSix());
        }
        
        List<Map> columnDefs = new ArrayList<Map>();
//        SysCodeFormbean sysCode = new SysCodeFormbean();
//        sysCode.setCodeName("六级");
//        sysCode.setCodeValue("6");
//        fieldList.add(sysCode);
        ...
        for (SysCodeFormbean sysCodeFormbean : fieldList) {
            ...
            if("5".equals(sysCodeFormbean.getCodeValue())){
                field = "fif";
                name = "五级";
                series.setData(fifData);
            }
//            if("6".equals(sysCodeFormbean.getCodeValue())){
//                field = "six";
//                name = "六级";
//                series.setData(sixData);
//            }
            map.put("field", field);
            map.put("name", name);
        ...
        logger.info("end searchDeptOperatCountByAsalev");
        return resValue.getJsonStr();
    }
     ...
     * @return
     */
    @RequestMapping(value = "/searchDeathCountByDept")
    @ResponseBody
    @ApiOperation(value="意外死亡例数统计",httpMethod="POST",notes="意外死亡例数统计")
    public String searchDeathCountByDept(@ApiParam(name="searchFormBean", value ="统计查询参数") @RequestBody SystemSearchFormBean searchFormBean){
        ResponseValue resp = new ResponseValue();
        List<AnaesDocObserveTimeCount> tableList = statisticsService.searchDeathCountByDept(searchFormBean);
        resp.put("tableList", tableList);
        return resp.getJsonStr();
    }
	...
	* 意外死亡、进入麻醉复苏室例数统计按麻醉医生
     ...
     * 术后镇痛统计按科室
     */
    @RequestMapping(value = "/searchDeptCountByAnalgesicQNZ")
    @ResponseBody
    @ApiOperation(value="术后镇痛统计",httpMethod="POST",notes="术后镇痛统计")
    public String searchDeptCountByAnalgesicQNZ(@ApiParam(name="searchFormBean", value ="统计查询参数") @RequestBody SearchFormBean searchFormBean) {
        ResponseValue resp = new ResponseValue();
        logger.info("begin searchDeptCountByAnalgesic");
        ...
        List<StaticDeptCountByAnalgesic> tableList = statisticsService.searchDeptCountByAnalgesicQNZ(searchFormBean);
        resp.put("tableList", tableList);
        logger.info("end searchDeptCountByAnalgesic");
        return resp.getJsonStr();
    }
	...
	public String searchAnaesMethodByDept(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
		...
		for (int i = 0; i < time.length; i++) {
			...
			time[i] = methodList.get(i).getAnaMedId();
		...
		for (int i = 0; i < methodList.size(); i++)
		{
            ...
            for (int j = 0; j < tableList.size(); j++)
            {
                ...
                data.add(StringUtils.isNotBlank(anaesValue) ? Float.valueOf(anaesValue) : 0);
                setMaxAndMin(compareMap, StringUtils.isNotBlank(anaesValue) ? Float.valueOf(anaesValue) : 0); 
            }
            seriesList.add(series);
		}
     ...
     * @throws ParseException 
     */
    @RequestMapping(value = "/searchOperByASALevelQNZ")
    @ResponseBody
    @ApiOperation(value="麻醉分级管理例数统计",httpMethod="POST",notes="麻醉分级管理例数统计")
    public String searchOperByASALevelQNZ(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchOperByASALevel");
        ResponseValue resp = new ResponseValue();
        ...
        for (int i = 0; i < startTimeList.size(); i++)
        {
            searchFormBean.setLeaveTo(null);
            searchFormBean.setPacuLeaveTo(null);
            searchFormBean.setStartTime(startTimeList.get(i));
            searchFormBean.setEndTime(endTimeList.get(i));
            List<SearchOperByASALevel> asaLevelList = statisticsService.searchOperByASALevel(searchFormBean);
            vo = new HashMap<String, String>();
            ...
            searchFormBean.setLeaveTo("4");
            searchFormBean.setPacuLeaveTo("4");
            asaLevelList = statisticsService.searchDeathOperByASALevel(searchFormBean);
            for (int j = 0; j < asaLevelList.size(); j++)
            {
                SearchOperByASALevel searchOperByASALevel = asaLevelList.get(j);
                value[i][j * 2 + 1] = searchOperByASALevel.getTotal() + "";
    ...
    /**
     * 
     * @discription 实施镇痛治疗例数(黔南州人民医院)
     * @author zhouyi
     * @param searchConditionFormBean
     * @return
     * @throws ParseException 
     */
    @RequestMapping(value = "/searchOperByAnalgesicQNZ")
    @ResponseBody
    @ApiOperation(value="实施镇痛治疗例数",httpMethod="POST",notes="实施镇痛治疗例数")
    public String searchOperByAnalgesicQNZ(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchOperByAnalgesic");
        ResponseValue resp = new ResponseValue();
        ...
        for (int i = 0; i < startTimeList.size(); i++)
        {
            ...
            Integer total = statisticsService.searchOperByAnalgesicQNZ(searchFormBean);
            value[i][0] = total + "";
            vo.put(columnAry[0], total + "");
        ...
        List<String> endTimeList, int length) throws ParseException
    {
        ...
        if ("1".equals(searchConditionFormBean.getTimeType()))
        {
            ...
            {
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfMonth(DateUtils.addMonths(new Date(), -1))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfMonth(DateUtils.addMonths(new Date(), -1))));
            }
            //最近两个月
            else if ("2".equals(searchConditionFormBean.getTimeRang()))
            {
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfMonth(DateUtils.addMonths(new Date(), -2))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfMonth(DateUtils.addMonths(new Date(), -2))));
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfMonth(DateUtils.addMonths(new Date(), -1))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfMonth(DateUtils.addMonths(new Date(), -1))));
            }
        }
        ...
        else if ("2".equals(searchConditionFormBean.getTimeType()))
        {
            ...
            {
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfQuarter(DateUtils.addMonths(new Date(), -3))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfQuarter(DateUtils.addMonths(new Date(), -3))));
            }
            //最近两个季度
            else if ("2".equals(searchConditionFormBean.getTimeRang()))
            {
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfQuarter(DateUtils.addMonths(new Date(), -6))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfQuarter(DateUtils.addMonths(new Date(), -6))));
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfQuarter(DateUtils.addMonths(new Date(), -3))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfQuarter(DateUtils.addMonths(new Date(), -3))));
            }
            //最近三个季度
            ...
            {
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfQuarter(DateUtils.addMonths(new Date(), -9))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfQuarter(DateUtils.addMonths(new Date(), -9))));
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfQuarter(DateUtils.addMonths(new Date(), -6))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfQuarter(DateUtils.addMonths(new Date(), -6))));
                startTimeList.add(DateUtils.DateToString(DateUtils.getFirstDayOfQuarter(DateUtils.addMonths(new Date(), -3))));
                endTimeList.add(DateUtils.DateToString(DateUtils.getLastDayOfQuarter(DateUtils.addMonths(new Date(), -3))));
        ...
        for (int i = 0; i < startTimeList.size(); i++)
        {
            ...
            {
                ary[i] = startTimeList.get(i).substring(0, 4);
            }
            else if ("4".equals(searchConditionFormBean.getTimeType()))
            {
    ...
    public String searchHomeOperTimes(@ApiParam(name="searchFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchFormBean) {
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            if (null != user && ("ANAES_DOCTOR".equals(user.getRoleType()) || "ANAES_DIRECTOR".equals(user.getRoleType())))
            {
                count = statisticsService.searchHomeQMTimes(startTime, endTime, loginName, beid);
            }
            else if(null != user && ("NURSE".equals(user.getRoleType()) || "HEAD_NURSE".equals(user.getRoleType())))
            {
                int qmCount = statisticsService.searchHomeQMTimes(startTime, endTime, loginName, beid);
                int jmCount = statisticsService.searchHomeNurseJMTimes(startTime, endTime, loginName, beid);
    ...
    @RequestMapping(value = "/getPharmacyReport")
    @ResponseBody
    public String getPharmacyReport(@RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin getPharmacyReport");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setBeid(getBeid());
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        searchFormBean.setEndTime(searchConditionFormBean.getEndTime());
        List<GetPharmacyReportFormBean> list = statisticsService.getPharmacyReport(searchFormBean);
    ...
    @RequestMapping(value = "/searchNoStart9AndNoEnd18")
    @ResponseBody
    public String searchNoStart9AndNoEnd18(@RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchNoStart9AndNoEnd18");
        ResponseValue resp = new ResponseValue();
        ...
        for (int i = 0; i < startTimeList.size(); i++)
        {
            ...
            //9点未开始
            Integer total = statisticsService.searchOperNoStartAt9(searchFormBean);
            value[i][0] = total + "";
            vo.put(columnAry[0], total + "");
            
            //16点未结束
            Integer total1 = statisticsService.searchOperNoEndAt18(searchFormBean);
            value[i][1] = total1 + "";
            vo.put(columnAry[1], total1 + "");
        ...
        if (tableList.size() > 0)
        {
            ...
            series.setData(fdata);
            series.setType("bar");
            series.setName("9点未开始和18点未结束手术例数");
            seriesList.add(series);
        ...
        resp.put("columnAry", columnAry);
        resp.put("value", value);
        logger.info("end searchNoStart9AndNoEnd18");
        return resp.getJsonStr();
    }
     ...
     * @return
     */
    @RequestMapping(value = "/searchAnesDetailByCond")
    @ResponseBody
    @ApiOperation(value="麻醉科麻醉例数统计",httpMethod="POST",notes="麻醉科麻醉例数统计")
    public String searchAnesDetailByCond(@ApiParam(name="searchFormBean", value ="统计查询参数") @RequestBody SearchFormBean searchFormBean) {
        logger.info("begin searchAnesDetailByCond");
        ResponseValue resp = new ResponseValue();

        if (StringUtils.isBlank(searchFormBean.getBeid())) {
            searchFormBean.setBeid(getBeid());
        }
        resp.put("resultList", statisticsService.searchAnesDetailByCond(searchFormBean));
        logger.info("end searchAnesDetailByCond");
        return resp.getJsonStr();
    }
}
