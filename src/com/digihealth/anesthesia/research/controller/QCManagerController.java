...
public class QCManagerController extends BaseController{
    ...
    public String searchDoctorPatientRate(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        ...
        // 在岗麻醉医师总数
        int anestheDocNum = basUserService.selectAnaesDocNum();
        int operTotal = 0;
    ...
    public String searchOperCancleRate(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        ...
        if (operList != null && operList.size() > 0)
        {
        	operList.addAll(operCancleList);
            operTotal = operList.size();
            for (int i = 0; i < operList.size(); i++)
            {
                vo = new HashMap<String, String>();
    ...
    public String searchNonPlanToICURate(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        ...
        List<SearchOperFormBean> toICUOperList = qcManagerService.searchToICUOper(searchFormBean);
        ...
        // 表格统计数据的list
        List operTableList = new ArrayList();
        if (toICUOperList != null && toICUOperList.size() > 0)
        {
            toICUOperTotal = toICUOperList.size();
            for (int i = 0; i < toICUOperList.size(); i++)
            {
                vo = new HashMap<String, String>();
                vo.put("行名", (i + 1) + "");
                vo.put(NPCOLUMN[0], toICUOperList.get(i).getName());
                vo.put(NPCOLUMN[1], toICUOperList.get(i).getSex());
                vo.put(NPCOLUMN[2], toICUOperList.get(i).getAge());
                vo.put(NPCOLUMN[3], toICUOperList.get(i).getHid());
                vo.put(NPCOLUMN[4], toICUOperList.get(i).getOperaDate());
                vo.put(NPCOLUMN[5], toICUOperList.get(i).getOperatorName());
                vo.put(NPCOLUMN[6], toICUOperList.get(i).getDesignedOptName());
                vo.put(NPCOLUMN[7], toICUOperList.get(i).getAnaesMethodName());
                vo.put(NPCOLUMN[8], toICUOperList.get(i).getAnesthetistName());
                vo.put(NPCOLUMN[9], toICUOperList.get(i).getCircunurseName());
                vo.put(NPCOLUMN[10], toICUOperList.get(i).getPlanTo());
                vo.put(NPCOLUMN[11], toICUOperList.get(i).getActualTo());
                operTableList.add(vo);
            }
        }
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchNonPlanToICURateQNZ")
    @ResponseBody
    @ApiOperation(value="非计划转入ICU率",httpMethod="POST",notes="非计划转入ICU率")
    public String searchNonPlanToICURateQNZ(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchNonPlanToICURate");
        ResponseValue resp = new ResponseValue();
        Map<String, String> timeMap = new HashMap<String, String>();
        setTimeMap(searchConditionFormBean, resp, timeMap);
        ...
        //List<SearchOperFormBean> toICUQmOperList = qcManagerService.searchQmIcuNum(searchFormBean);
        //List<SearchOperFormBean> toICUJmOperList = qcManagerService.searchJmIcuNum(searchFormBean);
        List<SearchOperFormBean> toICUOperList = qcManagerService.searchToICUOperQNZ(searchFormBean);
        
        //非计划转入ICU
        List<SearchOperFormBean> toICUQmFjhOperList = qcManagerService.searchQmFjgIcuNumQNZ(searchFormBean);
        //toICUQmOperList.addAll(toICUJmOperList);
        int toICUOperTotal = 0;
    ...
    public String searchVenipunctyreRate(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        ...
        //在麻醉总结中有中心静脉穿刺(麻醉总结单体现)
        List<SearchOperFormBean> venipunctyreList = qcManagerService.searchOptVenipunctyreList(searchFormBean);
        Integer venListTotal = venipunctyreList.size();
        resp.put("venipunctyreList", venipunctyreList);
        resp.put("venListTotal", venListTotal);
        
        //同期中心静脉穿刺总例数(麻醉记录单中的麻醉事件体现)
        List<SearchOperFormBean> operList = qcManagerService.searchVenipunctyreList(searchFormBean);
        Integer operTotal = operList.size();
        resp.put("operList", operList);
        resp.put("operTotal", operTotal);
        
        float rate = 0.000f;
        if (venListTotal != 0 && operTotal != 0)
        {
            rate = (float)venListTotal / operTotal;
        }
        DecimalFormat df = new DecimalFormat("0.0000");
        BigDecimal b = new BigDecimal(df.format(rate));
        BigDecimal c = new BigDecimal(Double.valueOf(1000));
    ...
    public String searchAnesComaRate(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        ...
        List<SearchOperFormBean> regOptList = qcManagerService.searchOperByConsciousness(searchFormBean, null);
        
        //持续昏迷超过24小时
        List<SearchOperFormBean> comaList = qcManagerService.searchOperByConsciousness(searchFormBean, "是");
        		...
        		if(extList.size()>0){
        			...
        			for (DocPostFollowSpinal postFollowSpinal : extList) {
        				if((null == postFollowSpinal.getCognitObstacle() || postFollowSpinal.getCognitObstacle()==0) 
        					...
        					if(Objects.equal(postFollowSpinal.getSevereHeadache(), 1)){
        						contents+="重度头痛,";
        					}
        					if(Objects.equal(postFollowSpinal.getLeftLimbsFeelImp(), 1)){
        						contents+="麻木,";
        					}
        					if(Objects.equal(postFollowSpinal.getRightLimbsFeelImp(),1)){
        						contents+="异感,";
        					}
        					if(Objects.equal(postFollowSpinal.getLeftMoveFeelImp(),1)){
        						contents+="肌无力,";
        					}
        					if(Objects.equal(postFollowSpinal.getRightMoveFeelImp(),1)){
        						contents+="瘫痪,";
        					}
    ...
    public String searchGenAnesHoarseRate(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        ...
        List<SearchOperFormBean> extubatList = qcManagerService.searchOperByHoarse(searchFormBean, "是", null);
        /*for (SearchOperFormBean searchOperFormBean : regOptList) {
        	...
        	if(anaesevent!=null){
        		...
        		if(extList.size()>0){
        			...
        			if(null !=hoarse && hoarse.intValue()==2){
         ...
         * 同期全麻气管插管总例数
         */
        List<SearchOperFormBean> tubesList = qcManagerService.searchOperByHoarse(searchFormBean, null, "1");
