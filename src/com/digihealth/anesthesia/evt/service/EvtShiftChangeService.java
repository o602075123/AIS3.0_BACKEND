	...
	public void saveShiftChange(EvtShiftChange shiftChange, ResponseValue respValue) {
		...
		if (shiftChange.getId() != null) {
			...
			if (StringUtils.isBlank(shiftChange.getShiftChangePeopleId())) {
                ...
                Date shiftChangeTime = shiftChange.getShiftChangeTime();
                if (null == shiftChangeTime)
                {
                ...
                else
                {
                    ...
                    {
                        if (shiftChangeTime.getTime() <= shiftList.get(shiftList.size() - 1).getShiftChangeTime().getTime())
                        {
                            respValue.setResultCode("10000001");
                            respValue.setResultMessage("交接班时间不能晚于最后一次交接班时间");
                            return;
                        }
                    } 
                }
