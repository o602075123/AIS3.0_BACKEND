...
public class EvtOtherEventService extends BaseService {
	...
	@Transactional
    public void saveOthereventQNZ(@RequestBody EvtOtherEvent otherevent) {
        
        if (StringUtils.isBlank(otherevent.getEventDefId()))
        {
            BasOtherEvent oe = new BasOtherEvent();
            oe.setBeid(getBeid());
            oe.setEnable(1);
            oe.setId(GenerateSequenceUtil.generateSequenceNo());
            oe.setName(otherevent.getTitle());
            oe.setPinyin(PingYinUtil.getFirstSpell(oe.getName()));
            basOtherEventDao.insert(oe);
            
            otherevent.setEventDefId(oe.getId());
        }
