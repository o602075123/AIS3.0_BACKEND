			...
			if (part.getRole().equals("A")) {
				...
				for (EvtParticipant participant1 : participantList) {
					...
					for (EvtParticipant participant : mainAnesDocList) {
						...
						if (StringUtils.isNotBlank(participant1.getUserLoginName()) && participant.getUserLoginName().equals(participant1.getUserLoginName())) {
							flag = false;
							break;
						}
					}
				...
				Boolean delMainDocFlag = true; //是否删除主刀医生06
				for (EvtParticipant participant1 : participantList) {
					boolean flag = true;
					...
					if("06".equals(participant1.getOperatorType())){
						delMainDocFlag = false;
					}
				}
				//删除主刀医生
				if(delMainDocFlag){
					part.setOperatorType("06");
					evtParticipantDao.deleteByOperatorType(part);
				}
